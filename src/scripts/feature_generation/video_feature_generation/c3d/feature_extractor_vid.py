...
if __name__ == "__main__":
	...
	parser.add_argument('-o', '--OUTPUT_DIR', dest='OUTPUT_DIR', type=str, default=C3D_VIDEO_FEATURES_DIRECTORY, help='Output file name')
	parser.add_argument('-l', '--EXTRACTED_LAYER', dest='EXTRACTED_LAYER', type=int, choices=[5, 6, 7], default=6, help='Feature extractor layer')
	parser.add_argument('-i', '--VIDEO_DIR', dest='VIDEO_DIR', type = str, default=PROCESSED_VIDEOS_DIRECTORY, help='Input Video directory')
	parser.add_argument('-gpu', '--gpu', dest='GPU', action = 'store_true', help='Run GPU?', default='0')
	parser.add_argument('--OUTPUT_NAME', default=C3D_FEATURES_FILENAME, help='The output name of the hdf5 features')
	parser.add_argument('-b', '--BATCH_SIZE', default=6, help='the batch size')
	parser.add_argument('-id', '--gpu_id', default=0, type=int)
	parser.add_argument('-p', '--video_list_file', type=str, help='the video name list', default=VIDEO_LIST_PATH)
